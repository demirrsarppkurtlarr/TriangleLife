...
const TABS: { id: GameTab; label: string; icon: typeof Heart }[] = [
  ...
  { id: "yasam", label: "Yaşam", icon: Compass },
  ...
  return (
    <>
      ...
      >
        <div
          className="pointer-events-auto inline-flex items-center gap-0.5 rounded-full
            ...
            p-1.5 max-w-[95vw] overflow-x-auto"
        >
          {TABS.map(({ id, label, icon: Icon }) => {
            ...
            return (
              <button
                ...
                onClick={() => onChange(id)}
                className={cn(
                  "relative flex items-center gap-2 rounded-full px-3 py-2 text-[12px] font-medium transition-all duration-200 whitespace-nowrap",
                  isActive
      ...
      <nav
        ...
        aria-label="Ana menü"
      >
        <div className="grid grid-cols-5 gap-0 px-0.5 pt-1 pb-1">
          {TABS.map(({ id, label, icon: Icon }) => {
            const isActive = active === id;
            return (
              <button
